...
const TodoList = () => {
  ...
  const addTask = () => {
    if (task !== "") {
      ...
      setDoneTask([...doneTask, { task, completed: false }]);
        ...
        <ul className="list-group">
          ...
          {arrayTodos.map((item, index) => (
            <li className="list-group-item" key={index}>
              <i
                onClick={() => {
                  const deleteAnswer = prompt(
                    "Do you want to delete the task? Yes/No"
                  );
                  return deleteAnswer.toLowerCase() === "yes"
                    ? deleteTask(index)
                    : "";
                }}
